...
const formatIDR = n => "Rp " + n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
const loadCart = () => JSON.parse(localStorage.getItem("cart") || "[]");
const saveCart = cart => localStorage.setItem("cart", JSON.stringify(cart));
const updateCartCount = () => document.querySelectorAll("#cart-count").forEach(e => e.textContent = loadCart().length);
...
function updateNavbarPhoto() {
  ...
  navUser.textContent = username || email.split("@")[0];
  navPfp.src = savedPhoto || DEFAULT_ICON;
}
...
async function fetchNavbarUser() {
  ...
  try {
    ...
    if (pfp) pfp.src = data.photoUrl || DEFAULT_ICON;
...
function addToCart(id) {
  ...
  saveCart(cart);
  updateCartCount();
  alert("🛍️ Produk ditambahkan ke keranjang!");
}
...
function renderCart() {
  ...
  totalElement.textContent = formatIDR(total);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnCheckout) {
    btnCheckout.onclick = async () => {
      ...
      localStorage.setItem("lastOrder", JSON.stringify({ userEmail: email, nama, alamat, telepon, items, total, createdAt: new Date().toISOString() }));
  ...
  const pfp = document.getElementById("pfp");
  const navPfp = document.getElementById("nav-pfp");
  const fileInput = document.getElementById("photo-input") || document.getElementById("file-input");
  const deleteBtn = document.getElementById("delete-photo-btn") || document.getElementById("delete-photo");
  const email = localStorage.getItem("email");
  const savedPhoto = localStorage.getItem("photoUrl");

  if (pfp) pfp.src = savedPhoto || DEFAULT_ICON;
  ...
  if (fileInput && email) {
    fileInput.addEventListener("change", async () => {
      ...
      const res = await fetch(`${API}/api/user/photo`, { method: "POST", body: formData });
      const data = await res.json();

      if (res.ok) {
        localStorage.setItem("photoUrl", data.photoUrl);
        if (pfp) pfp.src = data.photoUrl;
        if (navPfp) navPfp.src = data.photoUrl;
      }
      alert(data.message);
    });
  }
